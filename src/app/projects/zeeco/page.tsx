...
const documents = [
  { number: 'TOBE-FEED-PKG-001', title: 'FEED Package Transmittal', file: 'TOBE-FEED-PKG-001.pdf', desc: 'Complete front-end engineering design package (26 pages)', size: '4.2 MB' },
  { number: 'TOBE-PFD-001', title: 'Process Flow Diagram', file: 'TOBE-PFD-001.pdf', desc: 'Full process flow with energy balance and stream data', size: '1.8 MB' },
  { number: 'TOBE-PID-001', title: 'P&ID - Electrolyzer Skid', file: 'TOBE-PID-001.pdf', desc: 'Piping and instrumentation diagram for the electrolyzer system', size: '2.1 MB' },
  { number: 'TOBE-EL-001', title: 'Electrical One-Line', file: 'TOBE-EL-001.pdf', desc: 'Electrical distribution one-line diagram (480VAC)', size: '1.4 MB' },
  { number: 'TOBE-GA-001', title: 'General Arrangement', file: 'TOBE-GA-001.pdf', desc: 'Site layout and equipment general arrangement drawings', size: '3.6 MB' },
];
...
{`┌─── PROJECT 001 ─── ZEECO ARC DEPLOYMENT ────────────────┐`}
          ...
          <motion.div
            ...
            className="mb-4 inline-block rounded border border-[#00ff88]/40 bg-[#00ff88]/10 px-3 py-1"
          >
          ...
          >
            {[
              { label: 'UNITS', value: '12× T-25', color: '#00ff88' },
              { label: 'TOTAL POWER', value: '600 kW', color: '#00d4ff' },
              { label: 'H₂ OUTPUT', value: '300 kg/day', color: '#00ff88' },
...
{`┌─── PROJECT OVERVIEW ─────────────────────┐`}
          ...
          >
            <p>
              <span className="text-[#00ff88]">Zeeco Inc.</span> is the world&apos;s largest combustion
              equipment manufacturer and operates one of the largest combustion R&D centers globally.
              Their <span className="text-[#00d4ff]">Applied Research Center (ARC)</span> in Broken Arrow,
              Oklahoma is the site of Tobe Energy&apos;s first commercial hydrogen deployment.
            </p>
            <p>
              This installation deploys <span className="text-[#00ff88]">12× T-25 electrolyzers</span> producing{' '}
              <span className="text-[#00ff88]">300 kg/day</span> of green hydrogen — the first commercial
              validation of membrane-free electrolysis at scale. Phase 2 adds a{' '}
              <span className="text-[#00d4ff]">T-125 validation unit</span> (250 kW, 125 kg/day) to demonstrate
              the next-generation platform.
            </p>
            <p className="text-white/50">
              Zeeco currently pays <span className="text-[#ff6b35]">$30/kg</span> for delivered hydrogen.
              Tobe produces at <span className="text-[#00ff88]">&lt;$5/kg</span>.
            </p>
          </motion.div>
        </div>
      </section>
            ...
            <div className="divide-y divide-white/5">
              {specs.map((s, i) => (
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-[#00ff88]">{s.value}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
...
{`┌─── PROCESS FLOW ─────────────────────────┐`}
          ...
          <motion.div
            ...
            variants={fadeUp}
          >
            <pre className="overflow-x-auto rounded border border-white/10 bg-white/[0.02] p-6 text-[0.6rem] leading-relaxed text-[#00ff88]/80 sm:text-xs" style={{ whiteSpace: 'pre' }}>
{`╔══════════════════════════════════════════════════════════════════════════╗
║  ZEECO ARC — PROCESS FLOW DIAGRAM                                      ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                        ║
║  `}<span className="text-[#00d4ff]">{`WATER TREATMENT (Zeeco Scope)`}</span>{`                                       ║
║  ┌──────────┐    ┌──────────┐    ┌──────────┐                          ║
║  │  RO-101  │───▶│  DI-101  │───▶│  TK-101  │                          ║
║  │ 8000kg/d │    │ Polishing│    │  Buffer  │                          ║
║  │ 80% recv │    │          │    │          │                          ║
║  └──────────┘    └──────────┘    └────┬─────┘                          ║
║                                       │                                ║
║  `}<span className="text-[#00ff88]">{`ELECTROLYSIS (Tobe Scope)`}</span>{`            ▼                                ║
║  ┌───────────────────────────────────────┐                              ║
║  │  EL-101  ·  12× T-25 Electrolyzers   │                              ║
║  │  600 kW  ·  PLC-101 Controls          │                              ║
║  └─────────────────┬─────────────────────┘                              ║
║                    │                                                    ║
║  `}<span className="text-[#ff6b35]">{`GAS SEPARATION`}</span>{`   ├────────────────┐                                  ║
║  ┌──────────┐      │          ┌──────────┐                              ║
║  │  SEP-101 │◀─────┘          │  SEP-102 │                              ║
║  │    H₂    │                 │    O₂    │                              ║
║  └────┬─────┘                 └──────────┘                              ║
║       │                                                                ║
║  `}<span className="text-[#00d4ff]">{`PURIFICATION`}</span>{`                                                         ║
║  ┌──────────┐    ┌──────────┐    ┌──────────┐                          ║
║  │  PU-101  │───▶│  DR-101  │───▶│ ≥99.99%  │                          ║
║  │Cat. DeOx │    │Mole Sieve│    │  Dry H₂  │                          ║
║  └──────────┘    └──────────┘    └────┬─────┘                          ║
║                                       │                                ║
║                         ┌─────────────┼─────────────┐                  ║
║                         ▼             │             ▼                  ║
║                   `}<span className="text-[#ff6b35]">{`Phase 1`}</span>{`          │       `}<span className="text-[#00ff88]">{`Phase 2`}</span>{`                  ║
║                 ┌──────────┐          │     ┌──────────┐              ║
║                 │  FL-101  │          │     │  VS-101  │              ║
║                 │  Flare   │          │     │  T-125   │              ║
║                 │ (Zeeco)  │          │     │ 250 kW   │              ║
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {documents.map((doc, i) => (
              <motion.a
                ...
                custom={i}
                variants={fadeUp}
                className="group block rounded border border-white/10 bg-white/[0.02] p-5 transition-colors hover:border-[#00d4ff]/30 hover:bg-[#00d4ff]/[0.03]"
              >
                <div className="mb-2 flex items-start justify-between">
                  <span className="text-[0.65rem] tracking-[0.15em] text-[#00d4ff]">{doc.number}</span>
                  <span className="text-[0.6rem] text-white/30">{doc.size}</span>
                </div>
                <p className="mb-1 text-sm font-semibold text-white/90 group-hover:text-[#00d4ff]">
                  {doc.title}
                </p>
                <p className="mb-3 text-xs text-white/40">{doc.desc}</p>
                <div className="flex items-center gap-2 text-xs text-white/30 group-hover:text-[#00d4ff]/60">
                  <span>↓</span>
...
{`┌─── SCOPE OF WORK ────────────────────────┐`}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              <div className="mb-4 border-b border-[#00ff88]/20 pb-2">
                <span className="text-xs font-bold tracking-[0.2em] text-[#00ff88]">TOBE SCOPE</span>
              </div>
            ...
            >
              <div className="mb-4 border-b border-[#00d4ff]/20 pb-2">
                <span className="text-xs font-bold tracking-[0.2em] text-[#00d4ff]">ZEECO SCOPE</span>
              </div>
              <div className="space-y-3">
